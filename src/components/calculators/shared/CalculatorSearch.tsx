...
// Mortgage calculators
{
  ...
  description: "Calculate your DTI ratio to see if you qualify for a mortgage",
  category: "Mortgage",
...
// Retirement calculators
{
  ...
  description: "Estimate the cost of early withdrawals from retirement accounts",
  category: "Retirement",
...
// Tools calculators
{
  ...
  category: "Tools",
  tags: ["currency", "exchange", "conversion", "money", "forex", "international"],
  ...
  category: "Tools",
  tags: ["measurement", "conversion", "units", "length", "weight", "volume", "temperature"],
...
const CalculatorSearch = () => {
  ...
  useEffect(() => {
    ...
    const filtered = allCalculators.filter(calculator => {
      return calculator.name.toLowerCase().includes(term) || calculator.description.toLowerCase().includes(term) || calculator.category.toLowerCase().includes(term) || calculator.tags.some(tag => tag.toLowerCase().includes(term));
    });
  ...
  return <div className="relative mx-auto w-full">
      <div onClick={toggleCommandMenu} className="relative flex items-center rounded-md border border-input bg-white py-2 cursor-pointer px-0 ">
        <Search className="h-4 w-4 mr-2 text-muted-foreground" />
        <span className="text-muted-foreground flex-1">Search calculators...</span>
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        <CommandInput placeholder="Search for calculators..." value={searchQuery} onValueChange={setSearchQuery} />
        <CommandList>
          <CommandEmpty>No calculators found.</CommandEmpty>
          {filteredCalculators.length > 0 && <CommandGroup heading="Calculators">
              {filteredCalculators.map(calculator => <CommandItem key={calculator.id} onSelect={() => handleSelect(calculator.path)} disabled={calculator.comingSoon}>
                  <div className="flex flex-col">
                    <span className="font-medium">{calculator.name}</span>
                    <span className="text-sm text-muted-foreground">
                      {calculator.description} <span className="text-xs text-primary">({calculator.category})</span>
                    </span>
                  </div>
                </CommandItem>)}
            </CommandGroup>}

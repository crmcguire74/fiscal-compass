...
import { Trash2, PlusCircle, TrendingDown, DollarSign, Percent, ListChecks, Info, BarChart3, ListOrdered, Save } from 'lucide-react'; // Consolidated icons
...
const DebtPayoffCalculator = () => {
  ...
  const [debts, setDebts] = useState<DebtItem[]>([
    ...
    { id: 'debt-1', name: 'Credit Card', balance: 5000, interestRate: 18, minimumPayment: 150 },
    { id: 'debt-2', name: 'Car Loan', balance: 12000, interestRate: 6, minimumPayment: 300 },
    { id: 'debt-3', name: 'Student Loan', balance: 20000, interestRate: 4.5, minimumPayment: 200 }
  ]);
  ...
  useEffect(() => {
    ...
    if (savedData) {
      ...
      if (savedData.extraPayment !== undefined) setExtraPayment(Number(savedData.extraPayment || 0));
      setDataStored(true);
      // Optionally restore previous results if saved
      if (savedData.snowballResult && savedData.avalancheResult) {
          setSnowballResult(savedData.snowballResult);
          setAvalancheResult(savedData.avalancheResult);
      }
    } else {
  ...
  // Auto-recalculate if results exist and inputs change
  useEffect(() => {
  ...
  const sortDebtsByMethod = (method: 'snowball' | 'avalanche', currentDebts: WorkingDebtItem[]) => {
  ...
  const calculatePayoff = (showToast = true) => {
    ...
    if (debts.some(debt => debt.balance < 0 || debt.interestRate < 0 || debt.minimumPayment < 0)) {
      toast({ title: "Invalid Input", description: "Debt values cannot be negative.", variant: "destructive" }); return;
    }
    if (debts.some(debt => debt.minimumPayment <= 0 && debt.balance > 0)) {
      toast({ title: "Missing Minimum Payment", description: "Please enter a minimum payment (>0) for all debts with a balance.", variant: "destructive" }); return;
    }
    if (extraPayment < 0) {
      toast({ title: "Invalid Extra Payment", description: "Extra payment cannot be negative.", variant: "destructive" }); return;
    }
    if (debts.length === 0) {
      toast({ title: "No Debts", description: "Please add at least one debt.", variant: "destructive" });
      setSnowballResult(null); setAvalancheResult(null); return;
    }
    ...
    if (showToast) {
        ...
        const timeDiff = Math.abs(snowball.monthsToPayoff - avalanche.monthsToPayoff);
        let description = `Snowball: ${snowball.monthsToPayoff} mo, ${formatCurrency(snowball.totalInterest)} interest. Avalanche: ${avalanche.monthsToPayoff} mo, ${formatCurrency(avalanche.totalInterest)} interest.`;
        if (interestDiff > 1 || timeDiff > 0) {
             const betterMethod = avalanche.totalInterest < snowball.totalInterest ? 'Avalanche' : 'Snowball';
             const timeComparison = timeDiff === 0 ? 'the same time' : `${timeDiff} ${timeDiff === 1 ? 'month' : 'months'} ${avalanche.monthsToPayoff < snowball.monthsToPayoff ? 'less' : 'more'}`;
             description = `${betterMethod} saves ${formatCurrency(interestDiff)} interest and takes ${timeComparison}.`;
        }
        toast({ title: "Payoff Plans Calculated", description: description, duration: 7000 });
    }
  };
  ...
  const calculatePayoffPlan = (method: 'snowball' | 'avalanche'): PayoffResult => {
    let workingDebts: WorkingDebtItem[] = debts.map(debt => ({
        ...debt, currentBalance: debt.balance, totalInterestPaid: 0
    }));
    let month = 0;
    ...
    const debtDetails = workingDebts.map(debt => ({
        id: debt.id, name: debt.name, payoffMonth: 0, totalInterestPaid: 0
    }));
    const originalTotalMinimums = debts.reduce((sum, d) => sum + d.minimumPayment, 0);
    ...
    while (workingDebts.some(d => d.currentBalance > 0.01)) {
        ...
        workingDebts.forEach(debt => {
            ...
            paymentTowardsThisDebt = Math.min(paymentTowardsThisDebt, debt.currentBalance + monthlyInterest); // Cap payment at amount needed

            const principalPaid = Math.max(0, paymentTowardsThisDebt - monthlyInterest); // Ensure principal isn't negative
        ...
        const totalPaidThisMonth = totalMinimumsPaidThisMonth + extraPayment + snowballAmount - paymentPool;
        totalPaidOverall += totalPaidThisMonth;
        const remainingBalanceOverall = workingDebts.reduce((sum, d) => sum + d.currentBalance, 0);

        payoffSchedule.push({
            month,
            remainingBalance: remainingBalanceOverall < 0.01 ? 0 : remainingBalanceOverall, // Ensure final is 0
        ...
        if (month > 1200) { console.error("Calculation exceeded 100 years."); break; } // Safety break
    }

    // Ensure final interest is captured for debts paid off on the last month
     debtDetails.forEach(detail => {
         if (detail.payoffMonth === month) {
             const finalDebtState = debts.map(d => ({...d, currentBalance: 0, totalInterestPaid: 0}))
                                         .find(d => d.id === detail.id); // Find original debt
             const calculatedDebt = calculatePayoffPlan(method).debtDetails.find(d => d.id === detail.id); // Re-run needed to get final interest state accurately
             if (calculatedDebt) {
                 detail.totalInterestPaid = calculatedDebt.totalInterestPaid;
             }
         }
     });
  ...
  return (
    <Card className="w-full shadow-lg border-gray-200">
      ...
      <CardContent className="pt-6 space-y-6">
                 ...
                 <Card className="border-dashed border-blue-300 h-full">
                    <CardHeader>
                        <CardTitle className="text-lg flex items-center gap-2 text-blue-700">
                         ...
                         <CardDescription>Additional amount paid monthly.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="space-y-2">
                            <Label htmlFor="extraPayment" className="flex items-center">Amount</Label>
                            <div className="flex items-center">
                                <span className="input-prefix">$</span>
                                <Input id="extraPayment" type="number" min="0" value={extraPayment}
                                    onChange={(e) => setExtraPayment(Number(e.target.value) || 0)}
                                    className="input-field rounded-l-none" placeholder="e.g., 100"
                                />
                            </div>
                        </div>
                    </CardContent>
                 ...
                 <Card className="border-dashed border-green-300 h-full">
                    <CardHeader>
                        <div className="flex justify-between items-center">
                         ...
                         <CardDescription>List all debts to include in the plan.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 -mr-2">
                            {debts.length === 0 && <p className="text-sm text-muted-foreground text-center py-4">No debts added.</p>}
                            {debts.map((debt) => (
                            <Card key={debt.id} className="p-3 bg-gray-50/50">
                                <div className="flex justify-between items-start mb-2">
                                    <Input value={debt.name} onChange={(e) => handleDebtChange(debt.id, 'name', e.target.value)}
                                        className="flex-1 mr-2 h-9 font-medium" placeholder="Debt name" />
                                    <TooltipProvider>
                                        <UITooltip delayDuration={100}>
                                            <TooltipTrigger asChild>
                                                <Button onClick={() => handleRemoveDebt(debt.id)} variant="ghost" size="icon"
                                                    className="text-red-500 hover:bg-red-100 h-9 w-9"> <Trash2 className="h-4 w-4" /> </Button>
                                            </TooltipTrigger> <TooltipContent><p>Remove Debt</p></TooltipContent>
                                        </UITooltip>
                                    </TooltipProvider>
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                    <div className="space-y-1">
                                        ...
                                        <div className="flex items-center"><span className="input-prefix h-9">$</span>
                                            <Input id={`balance-${debt.id}`} type="number" min="0" value={debt.balance}
                                                onChange={(e) => handleDebtChange(debt.id, 'balance', Number(e.target.value))}
                                                className="input-field rounded-l-none h-9" placeholder="0" />
                                        </div>
                                    </div>
                                    <div className="space-y-1">
                                        <Label htmlFor={`interest-${debt.id}`} className="text-xs">Rate</Label>
                                        <div className="flex items-center">
                                            <Input id={`interest-${debt.id}`} type="number" min="0" step="0.1" value={debt.interestRate}
                                                onChange={(e) => handleDebtChange(debt.id, 'interestRate', Number(e.target.value))}
                                                className="input-field rounded-r-none h-9" placeholder="0" />
                                            <span className="input-suffix h-9">%</span>
                                        </div>
                                    </div>
                                    <div className="space-y-1">
                                        <Label htmlFor={`minimum-${debt.id}`} className="text-xs">Min. Pymt</Label>
                                        <div className="flex items-center"><span className="input-prefix h-9">$</span>
                                            <Input id={`minimum-${debt.id}`} type="number" min="0" value={debt.minimumPayment}
        ...
        {/* Calculate Button */}
        <Button onClick={() => calculatePayoff(true)} className="w-full" size="lg" disabled={debts.length === 0}>
        ...
        {snowballResult && avalancheResult && (
          <div className="pt-6 border-t space-y-6">
             <h2 className="text-xl font-semibold text-center text-finance-primary">Payoff Plan Comparison</h2>
            <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'snowball' | 'avalanche')}>
              ...
              { [snowballResult, avalancheResult].map(result => (
                  <TabsContent key={result.method} value={result.method} className="space-y-4 mt-4">
                     <Card className="bg-gray-50 border-gray-200">
                         <CardHeader className="pb-2 pt-3">
                             <CardTitle className="text-base">{result.method === 'snowball' ? 'Snowball' : 'Avalanche'} Summary</CardTitle>
                         </CardHeader>
                         <CardContent className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-center">
                             <div className="p-2 border rounded bg-white"><div className="text-xs text-muted-foreground">Total Interest</div><div className="text-lg font-semibold">{formatCurrency(result.totalInterest)}</div></div>
                             <div className="p-2 border rounded bg-white"><div className="text-xs text-muted-foreground">Time to Debt-Free</div><div className="text-lg font-semibold">{result.monthsToPayoff} mo</div></div>
                             <div className="p-2 border rounded bg-white"><div className="text-xs text-muted-foreground">Total Paid</div><div className="text-lg font-semibold">{formatCurrency(result.totalPaid)}</div></div>
                         </CardContent>
                     </Card>
                  </TabsContent>
              ))}
              ...
              <Card>
                <CardHeader>
                    <CardTitle className="text-lg flex items-center gap-2">
                        <BarChart3 className="h-5 w-5" /> Balance Over Time ({activeTab === 'snowball' ? 'Snowball' : 'Avalanche'})
                    </CardTitle>
                </CardHeader>
                <CardContent>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        ...
                        <XAxis dataKey="month" label={{ value: 'Month', position: 'insideBottom', offset: -5 }} tick={{ fontSize: 10 }} interval="preserveStartEnd" tickFormatter={(tick) => tick % 12 === 0 ? `Yr ${tick/12}` : (tick % 6 === 0 ? `M${tick}`: '')}/>
                        <YAxis tickFormatter={(value) => `$${(value / 1000).toFixed(0)}k`} label={{ value: 'Balance', angle: -90, position: 'insideLeft', offset: 0 }} tick={{ fontSize: 10 }} width={60}/>
                        <Tooltip formatter={(value) => [formatCurrency(Number(value)), 'Balance']} labelFormatter={(label) => `Month ${label}`} contentStyle={{ fontSize: '12px' }}/>
                        <Bar dataKey="balance" fill="#3b82f6" name="Remaining Balance" radius={[4, 4, 0, 0]}/>
                        </BarChart>
                    </ResponsiveContainer>
                    </div>
                </CardContent>
            ...
            {activeResult && activeResult.debtDetails.length > 0 && (
                <Card>
                    <CardHeader>
                        <CardTitle className="text-lg flex items-center gap-2">
                            <ListOrdered className="h-5 w-5" /> Debt Payoff Order ({activeTab === 'snowball' ? 'Snowball' : 'Avalanche'})
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="border rounded-lg divide-y max-h-[300px] overflow-y-auto">
                            ...
                            .map((debt, index) => (
                                <div key={debt.id} className="p-3 flex flex-wrap justify-between items-center text-sm">
                                <div className="font-medium mr-2"><span className="inline-block text-center w-5 mr-1">{index + 1}.</span> {debt.name}</div>
                                <div className="text-xs text-muted-foreground text-right ml-auto">Paid off mo {debt.payoffMonth} ({formatCurrency(debt.totalInterestPaid)} int.)</div>
                                </div>
                            ))}
                        </div>
                    </CardContent>
      ...
      <CardFooter className="flex flex-col text-sm text-muted-foreground border-t pt-4 text-center space-y-1">
        <p><strong>Snowball:</strong> Pay off smallest balances first. <strong>Avalanche:</strong> Pay off highest interest rates first.</p>
        <p className="italic">Both methods assume minimum payments on all debts + extra payment applied according to the strategy.</p>
      </CardFooter>
    </Card>
  );
};

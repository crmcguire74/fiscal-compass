...
const TakeHomePayHourlyCalculator = () => {
  const [hourlyRate, setHourlyRate] = useState<number>(20);
  const [hoursPerWeek, setHoursPerWeek] = useState<number>(40);
  const [averageTipsPerWeek, setAverageTipsPerWeek] = useState<number>(0);
  ...
  const [payPeriods, setPayPeriods] = useState<number>(52); // Default to weekly for hourly
  // const [isBonus, setIsBonus] = useState<boolean>(false); // Remove bonus calculation for hourly
  ...
  // Calculate annual income and take-home pay
  useEffect(() => {
    const annualIncome = (hourlyRate * hoursPerWeek + averageTipsPerWeek) * 52;
    const calculationResults = calculateTakeHomePay(
      annualIncome,
      state,
      filingStatus,
      use401kPercent ? retirement401kPercent : 0,
      use401kPercent ? 0 : retirement401kAmount,
      otherTaxDeferred,
      useStandardDeduction,
      itemizedDeductions,
      payPeriods,
      false // isBonus is always false for hourly
    );
    setResults({ ...calculationResults, annualIncome }); // Add annualIncome to results for tooltip
  }, [
    hourlyRate,
    hoursPerWeek,
    averageTipsPerWeek,
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length && results?.annualIncome > 0) {
      return (
        <div className="bg-white p-4 border rounded shadow-sm">
          <p className="font-semibold">{payload[0].name}</p>
          <p>
            {formatCurrency(payload[0].value)} (
            {((payload[0].value / results.annualIncome) * 100).toFixed(1)}%)
          </p>
        </div>
      );
    }
  ...
  return (
    <Card className="w-full">
          ...
          <TabsContent value="annual" className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <div>
                ...
                <div className="flex items-center mt-1">
                  ...
                  <Input
                    ...
                    value={hourlyRate}
                    onValueChange={(val) =>
                      setHourlyRate(typeof val === "string" ? 0 : Number(val))
                    }
                    onClear={() => setHourlyRate(0)}
                    step="0.5"
                  />
                </div>
              </div>
              <div>
                ...
                <Input
                  ...
                  value={hoursPerWeek}
                  onValueChange={(val) =>
                    setHoursPerWeek(typeof val === "string" ? 0 : Number(val))
                  }
                  onClear={() => setHoursPerWeek(0)}
                  step="1"
                  min="0"
                />
              </div>
              <div>
                <Label htmlFor="averageTipsPerWeek">Avg Weekly Tips</Label>
                <div className="flex items-center mt-1">
                  <span className="text-gray-500 mr-2">$</span>
                  <Input
                    id="averageTipsPerWeek"
                    type="number"
                    value={averageTipsPerWeek}
                    onValueChange={(val) =>
                      setAverageTipsPerWeek(
                        typeof val === "string" ? 0 : Number(val)
                      )
                    }
                    onClear={() => setAverageTipsPerWeek(0)}
                    step="10"
                    min="0"
                  />
                </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <Select
                  value={payPeriods.toString()}
                  onValueChange={(value) => setPayPeriods(Number(value))}
                >
              ...
              <div className="space-y-4">
                  ...
                  {use401kPercent ? (
                    <div className="flex items-center gap-4">
                      <Slider
                        ...
                        step={0.5}
                        onValueChange={(value) =>
                          setRetirement401kPercent(value[0])
                        }
                        className="flex-1"
                      />
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Label htmlFor="standardDeduction">
                    ...
                    {formatCurrency(filingStatus === "single" ? 13850 : 27700)})
                  </Label>
                </div>
          ...
          <TabsContent value="paycheck" className="space-y-4">
            <div className="text-center p-6 bg-gray-50 rounded-lg">
              <div className="text-sm text-muted-foreground mb-2">
                To calculate per-paycheck amounts, switch to Annual view and set
                your pay frequency
              </div>
              <button
                ...
                <CardContent>
                  ...
                  <div className="mt-4">
                    <div className="text-xl font-bold">
                      {formatCurrency(results.takeHomePayPerPeriod)}
                    </div>
              ...
              <Card className="bg-gray-50">
                ...
                <CardContent>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="text-lg font-semibold">
                      Marginal Rate: {results.marginalTaxRate.toFixed(2)}%
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="bg-gray-50">
                ...
                <CardContent>
                  <div className="text-3xl font-bold text-blue-600">
                    {formatCurrency(
                      results.retirement401k + results.otherTaxDeferred
                    )}
                  </div>
                  <div className="text-sm text-muted-foreground">Annually</div>
                  ...
                  <div className="mt-4">
                    <div className="text-lg font-semibold">
                      401(k): {formatCurrency(results.retirement401k)}
                    </div>
                    <div className="text-lg font-semibold">
                      Other: {formatCurrency(results.otherTaxDeferred)}
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-blue-600">
                      {formatCurrency(results.retirement401k)}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-blue-600">
                      {formatCurrency(results.otherTaxDeferred)}
                    </span>
                  </div>
      ...
      <CardFooter className="border-t pt-4 text-sm text-muted-foreground">
        <p>
          This calculator provides estimates based on 2023 tax rates and does
          not account for all possible deductions or credits. For personalized
...
export default TakeHomePayHourlyCalculator;
